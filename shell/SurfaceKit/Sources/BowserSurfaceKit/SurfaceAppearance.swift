...
public enum SurfaceColorSpec {
    public static let variants: Set<String> = ["light", "dark", "high_contrast_light", "high_contrast_dark"]
    public static func valid(_ value: Any, depth: Int = 0) -> Bool {
    ...
    public static func system(_ role: String) -> NSColor? {
        switch role {
    ...
    public static func resolve(_ value: Any?, palette: [String: Any] = [:], dark: Bool, highContrast: Bool,
                        fallback: String = "text") -> NSColor {
        func find(_ value: Any, seen: Set<String>, depth: Int) -> NSColor? {
            guard depth < 16 else { return nil }
            if let name = value as? String {
                if let hex = SurfaceServices.color(hex: name) { return hex }
...
extension EnvironmentValues {
    public var surfacePalette: [String: Any] {
        get { self[SurfacePaletteKey.self] }
        set { self[SurfacePaletteKey.self] = newValue }
    }
}
public struct SurfaceColors: DynamicProperty {
    public init() {}
    @Environment(\.surfacePalette) public var palette
    @Environment(\.colorScheme) public var scheme
    @Environment(\.colorSchemeContrast) public var contrast
    public func native(_ value: Any?, fallback: String = "text") -> NSColor {
        SurfaceColorSpec.resolve(value, palette: palette, dark: scheme == .dark,
                                 highContrast: contrast == .increased, fallback: fallback)
    }
    public func color(_ value: Any?, fallback: String = "text") -> Color { Color(nsColor: native(value, fallback: fallback)) }
}

public struct SurfacePaletteScope: ViewModifier {
    public init(palette: [String: Any]?) { self.palette = palette }
    public let palette: [String: Any]?
