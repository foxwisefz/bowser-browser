...
// Bowser host spike (bowser-browser-8wt): minimal Rust host embedding Servo.
// Adapted from servo's `components/servo/examples/winit_minimal.rs` (MPL 2.0).
// The winit shell here is scaffolding — the real chrome is the Swift/AppKit
// shell (ADR 0003); this crate's job is owning the engine surface.
    ...
    fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        if let Self::Initial(waker) = self {
            ...
            let window = event_loop
                .create_window(Window::default_attributes().with_title("Bowser"))
            ...
            let url = Url::parse(START_URL).expect("Invalid START_URL");
            let webview =
